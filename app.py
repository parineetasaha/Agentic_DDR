...
def process_files(job_id, doc1_path, doc2_path, client_name):
    status = jobs[job_id]
    ...
    try:
        ...
        output_filename = f"{safe_name}_DDR_Report_{timestamp}.pdf"
        output_path = os.path.join(app.config['OUTPUT_FOLDER'], output_filename)
        build_pdf_report(ddr_data, all_images, output_path, client_name=client_name or "Client")
...
def analyze():
    ...
    thread = threading.Thread(target=process_files, args=(job_id, doc1_path, doc2_path, client_name))
    thread.daemon = True
    thread.start()

    return jsonify({"job_id": job_id, "client_name": client_name or "Client"})
